...
def create_tokenizer(args):
    ...
    logging.info(f'Tokenizer loaded: {args.backbone}')
    return tokenizer

def create_model(model_class, config, args):
    model = model_class.from_pretrained(args.backbone, config=config)
    logging.info('Model built and loaded from pretrained')
    return model
...
def evaluate(model, test_loader):
    ...
    for step, batch in enumerate(test_loader):
        ...
        if not isinstance(batch, dict):
            logging.error("Batch data is not a dictionary. Check data loader implementation.")
            continue
        
        # Move batch data to the appropriate device
        prepared_batch = {k: v.to(device) if isinstance(v, torch.Tensor) else v for k, v in batch.items()}
        
        with torch.no_grad():
            try:
                outputs = model.generate_step(prepared_batch)
                results.append(outputs)
                logging.info(f'Evaluated batch {step+1}/{len(test_loader)}')
            except KeyError as e:
                logging.error(f"Missing key in batch data: {e}. Batch keys: {list(batch.keys())}")
                break
...
def main():
    ...
    model = create_model(model_class, config, args)
    ...
    test_loader = get_loader(
        args,
        task_list={'sequential': ['1-1']},
        sample_numbers={'sequential': 1},
        split=args.test,
        mode='valid',
    ...
    logging.info('Evaluation complete and results saved.')
